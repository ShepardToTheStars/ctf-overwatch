...
(which zsh > /dev/null 2>&1 && echo -e "${GREEN}zsh is already installed! Skipping installation.") \
...
(id -u vanessa > /dev/null 2>&1 && echo -e "${GREEN}vanessa already exists! Skipping creation.") \
...
# setup my SSH key
echo -e "${BLUE}Adding ssh keys for vanessa"
